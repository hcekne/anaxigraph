...
export function renderCandidateCard(item, pattern) {
  ...
  return `<article class="panel pattern-result-card candidate-result-card">
    ...
    <div class="pattern-candidate-rank">${escapeHtml(language.queue_rank?.meaning || "No queue-rank explanation was supplied.")}</div>
    ${item.details ? candidateDetails(item.details) : ""}
    <div class="pattern-result-footer"><span>Deterministic candidate selection; no agent judgment yet.</span>
...
function candidateLanguage(item) {
  ...
  return {
    ...
    queue_rank: { meaning: "No trustworthy queue-rank explanation was supplied." },
  };
}
